...
public class ApprenantService implements IApprenantService{
    ...
    public boolean deleteApprenant(int numApprenant) throws Exception {
        unApprenantRepository.deleteById(numApprenant);

        if(unApprenantRepository.existsById(numApprenant)){
            throw new Exception("Erreur durant la suppression de l'apprenant");
        }
    ...
    public Set<ApprenantEntity> getTousLesApprenants() {
        ...
        return mesApprenants.stream().collect(Collectors.toSet());
    }
